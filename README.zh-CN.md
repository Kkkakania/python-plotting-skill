...
当前 main 分支保持小而清楚：20 个 Matplotlib 模板、合成数据、gallery 渲染脚本、来源说明和质量检查。不要声称它已经有用户规模、下载量或外部背书。
...
| `residual_convergence` | 迭代求解器或仿真残差衰减 |
| `main_inset` | 主趋势和局部放大区域 |
| `shared_colorbar_panels` | 共用色标的多个热力图 |
| `gantt_timeline` | 简单任务时间线 |
